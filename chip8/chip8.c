...
void chip8_emulatecycle() {
    ...
    switch (opcode & 0xF000) {
        ...
        case 0x1000: // 1nnn: jump to address nnn
            fprintf(stderr, "Jump to address 0x%x\n", nnn);
            PC = nnn;
            break;
        case 0x2000: // 2nnn: call address nnn
            fprintf(stderr, "Call address 0x%x\n", nnn);
            stack[SP++] = PC + 2;
            PC = nnn;
            break;
        ...
        case 0x6000: // 6xkk: set V[x] = kk
            fprintf(stderr, "Set V[0x%x] to 0x%x\n", x, kk);
            V[x] = kk;
            PC += 2;
            break;
        case 0x7000: // 7xkk: set V[x] = V[x] + kk
            fprintf(stderr, "Set V[0x%d] to V[0x%d] + 0x%x\n", x, x, kk);
            V[x] += kk;
            PC += 2;
            break;
        case 0x8000: // 8xyn: Arithmetic stuff
            switch (n) {
                case 0x0:
                    fprintf(stderr, "V[0x%x] = V[0x%x] = 0x%x\n", x, y, V[y]);
                    V[x] = V[y];
                    break;
                case 0x1:
                    fprintf(stderr, "V[0x%x] |= V[0x%x] = 0x%x\n", x, y, V[y]);
                ...
                case 0x3:
                    ...
                    break;
                case 0x4:
                    fprintf(stderr, "V[0x%x] = V[0x%x] + V[0x%x] = 0x%x + 0x%x\n", x, x, y, V[x], V[y]);
                ...
                case 0x5: 
                    ...
                    break;
                case 0x6:
                    fprintf(stderr, "V[0x%x] = V[0x%x] >> 1 = 0x%x >> 1\n", x, x, V[x]);
                    V[0xF] = V[x] & 0x1;
                    V[x] = (V[x] >> 1);
                    break;
                case 0x7:
        ...
        case 0xE000: // key-pressed events
            switch (kk) {
                case 0x9E: // skip next instr if key[Vx] is pressed
                    fprintf(stderr, "Skip next instruction if key[%d] is pressed\n", x);
                    PC += (key[V[x]]) ? 4 : 2;
                    break;
                case 0xA1: // skip next instr if key[Vx] is not pressed
                    fprintf(stderr, "Skip next instruction if key[%d] is NOT pressed\n", x);
                    PC += (!key[V[x]]) ? 4 : 2;
                    break;
                default:
                    unknown_opcode(opcode);
        ...
        case 0xF000: // misc
            switch (kk) {
                case 0x07:
                    fprintf(stderr, "V[0x%x] = delay timer = %d\n", x, delay_timer);
                    V[x] = delay_timer;
                    PC += 2;
                    break;
                case 0x0A:
                    ...
                    break;
                case 0x15:
                    fprintf(stderr, "delay timer = V[0x%x] = %d\n", x, V[x]);
                    delay_timer = V[x];
                    PC += 2;
                    break;
                case 0x18:
                    fprintf(stderr, "sound timer = V[0x%x] = %d\n", x, V[x]);
                    sound_timer = V[x];
                    PC += 2;
                    break;
                case 0x1E:
                    fprintf(stderr, "I = I + V[0x%x] = 0x%x + 0x%x\n", x, I, V[x]);
                    V[0xF] = (I + V[x] > 0xfff) ? 1 : 0;
                    I = I + V[x];
                    PC += 2;
                    break;
                case 0x29:
                    fprintf(stderr, "I = location of font for character V[0x%x] = 0x%x\n", x, V[x]);
                    I = FONTSET_BYTES_PER_CHAR * V[x];
                    PC += 2;
                    break;
                case 0x33:
                    fprintf(stderr, "Store BCD for %d starting at address 0x%x\n", V[x], I);
                    ...
                    break;
                case 0x55:
                    fprintf(stderr, "Copy sprite from registers 0 to 0x%x into memory at address 0x%x\n", x, I);
                    for (i = 0; i <= x; i++) { memory[I + i] = V[i]; }
                    I += x + 1;
                    PC += 2;
                    break;
                case 0x65:
                    fprintf(stderr, "Copy sprite from memory at address 0x%x into registers 0 to 0x%x\n", x, I);
